...
pub fn get_all_markets(client: RpcClient) -> Vec<(Pubkey, Market)>{
    let market_filter = RpcFilterType::Memcmp(Memcmp::new_raw_bytes(0, Market::discriminator().to_vec()));
    let result = client.get_program_accounts_with_config(&id(), RpcProgramAccountsConfig{
