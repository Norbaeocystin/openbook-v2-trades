...
pub fn get_owner_account_for_ooa(client: &RpcClient, ooa2owner: &BTreeMap<Pubkey, Pubkey> , key: &Pubkey) -> Option<Pubkey> {
    if !ooa2owner.contains_key(key) {
        let mut data = client.get_account_data(key).unwrap();
        if data.len() > 8 {
